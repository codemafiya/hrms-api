...
router.get("/getUploadedreferenceFiles:dtls", (req, res) => {
    ...
    mysqlPool.query(sql_getUploadedInfo, function (error, results) {
        if (error) {
            console.log("Error-->routes-->fileUpload-->getUploadedFiles--", error)
...
router.delete("/deleteUploadedFile:dtls", (req, res) => {
    ...
    mysqlPool.query(sql_delUpld, function (error, results) {
        if (error) {
            console.log("Error-->routes-->fileUpload-->deleteUploadedFile--", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        } else {
...
router.post('/uploadFile:dtl', function (req, res) {
    ...
    if (req.file != undefined) {
        console.log("Error-->routes-->fileUpload-->uploadFile--Investigate this error in upload--req is->", req);
        objectToSend["error"] = true;
                ...
                mysqlCon.beginTransaction(function (error1) {
                    if (error1) {
                        ...
                        mysqlCon.query(insertSql, function (error2, results2) {
                            if ((error2)) {
                                console.log("Error-->routes-->fileUpload-->uploadFile--", error2);
                                objectToSend["error"] = true;
                                ...
                                upload(req, res, function (err) {
                                    if ((err)) {
                                        console.log("Error-->routes-->fileUpload-->uploadFile--", err);
                                        objectToSend["error"] = true;
                                        objectToSend["data"] = "Server Side Error. Can't upload file at the moment "
                                        res.send(objectToSend)
                                        mysqlCon.rollback();
                                        ...
                                        try {
...

                                            {
                                                ...
                                                fs.copyFile(localFile, copyLoc, { recursive: true }, (err1) => {
                                                    if ((err1)) {
                                                        console.log("Error-->routes-->fileUpload-->uploadFile--", err1);
                                                        objectToSend["error"] = true;
                                                        objectToSend["data"] = "Server Side Error. Can't upload file at the moment "
                                                        res.send(objectToSend)
                                            ...
                                            console.log("Error-->routes-->fileUpload-->uploadFile--", ex);
                                            objectToSend["error"] = true;
                                            objectToSend["data"] = "Server Side Error. Can't upload file at the moment "
                                            res.send(objectToSend)
                                        }
