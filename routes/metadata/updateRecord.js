...
/* router.get('/getjournal:dtls', (req, res) => {
    ...
    mysqlPool.query(sql_fetchFldCode, function (error, results) {
        if (error) {
            console.log("Error-->routes-->projectMetadata-->updaterecord-->getjournal--", error)
...
router.get('/getrecordfields:dtls', (req, res) => {
...
router.put('/updateRecordStructure', (req, res) => {
    let obj = req.body;
    ...
    mysqlPool.query(sql_fetchFieldInfo, function (error, results) {
        if (error) {
            console.log("Error-->routes-->metadata-->updateRecord-->updateRecordStructure--", error)
            objectToSend["error"] = true
            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
            res.send(objectToSend);
        } else {
            ...
            if (addedColumns.length > 0) {
                ...
                for (let i = 0; i < addedColumns.length; i++) {
                    ...
                    if(tempObj["datatype_name"]=="serial"){
                        mainQuery += " add " + tempObj["field_technical_name"] + " bigint NOT NULL AUTO_INCREMENT,add INDEX("+tempObj["field_technical_name"]+")"
                    }else{
            ...
            mysqlPool.getConnection(function (error1, mysqlCon) {
                if (error1) {
                    console.log("Error-->routes-->metadata-->updateRecord-->updateRecordStructure--", error1)
                    objectToSend["error"] = true
                    objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
                    res.send(objectToSend);
                    ...
                    mysqlCon.beginTransaction(function (error2) {
                        if (error2) {
                            console.log("Error-->routes-->metadata-->updateRecord-->updateRecordStructure--", error2)
                            objectToSend["error"] = true
                            objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
                            res.send(objectToSend);
                            mysqlCon.release()
                            ...
                            mysqlCon.query(mainQuery, function (error3, results3) {
                                if (error3) {
                                    console.log("Error-->routes-->metadata-->updateRecord-->updateRecordStructure--", error3)
                                    objectToSend["error"] = true
                                    objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
                                    res.send(objectToSend);
                                    mysqlCon.rollback()
                                    ...
                                    if (isColAdded ||isColDeleted) {
                                        ...
                                        mysqlCon.commit(function (error4) {
                                            if (error4) {
                                                console.log("Error-->routes-->metadata-->updateRecord-->updateRecordStructure--", error4)
                                                objectToSend["error"] = true
                                                objectToSend["data"] = "Some error occured at server side. Please try again later. If problem persists, contact support."
                                                res.send(objectToSend);
                                                mysqlCon.release()
