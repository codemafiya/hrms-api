     ...
     try{
        mysqlPool.getConnection(function (err, mysqlCon) {
            if (err) {
               ...
               // console.log(row);
                let query="update " + propObj.svayamSystemDbName + ".user_info set email='"+row.email+"',first_name='"+row.first_name+"',"
                        +"last_name='"+row.last_name+"',work_phone_no_country_cd='"+row.work_phone_no_country_cd+"',work_phone_no='"+row.work_phone_no+"',"
                        +"work_email='"+row.work_email+"',country='"+row.country+"',designation='"+row.designation+"',state='"+row.state+"',city='"+row.city+"',"
                        +"postal_code='"+row.postal_code+"',address1='"+row.address1+"',address2='"+row.address2+"' where user_id="+row.user_id;
...
router.put('/changepassword',function(req,res){
    ...
    let query = "select password  from  "+propObj.svayamSystemDbName+".user_info where user_id= '" +row.user_id + "'"
    mysqlPool.query(query, function (error, results) {
        if (error) {
            ...
            if (origPassword != password) {
                ...
                let new_passwrd = JSON.stringify(encryptPassword(new_password));
                let insertquery ="update "+propObj.svayamSystemDbName+".user_info set password='"+new_passwrd+"' where user_id ='" +row.user_id + "'"
            ...
            objectToSend["error"] = false
            objectToSend["data"] ='Upadeted Sucessfully';
